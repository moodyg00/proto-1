...
const form = useForm({
  contact_id: props.workOrder.contact_id || '',
  service_id: props.workOrder.service_id || '',
  scheduled_date: props.workOrder.scheduled_date || '',
  assigned_contractor_id: props.workOrder.assigned_contractor_id || '',
  special_instructions: props.workOrder.special_instructions || '',
  notes: props.workOrder.notes || [],
});

const submit = () => form.put(`/operations/work-orders/${props.workOrder.id}`);
</script>
...
<template>
  <AppLayout>
    ...
    <div class="mx-auto max-w-3xl rounded-xl bg-white p-6 shadow-sm">
      <h2 class="text-2xl font-semibold text-slate-900">Edit {{ workOrder.work_order_number }}</h2>
      ...
      <form class="mt-6 space-y-4" @submit.prevent="submit">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <label class="text-sm font-medium text-slate-700">Assigned Contractor</label>
        ...
        <div class="flex items-center gap-3 pt-2">
          <button class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white" :disabled="form.processing">Update</button>
          <Link :href="`/operations/work-orders/${workOrder.id}`" class="text-sm text-slate-600">Cancel</Link>
        </div>
      </form>
    </div>
  </AppLayout>
